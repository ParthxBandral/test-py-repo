...
export default function WorkoutPlannerPage() {
  ...
  function handleGenerate(e: React.FormEvent) {
    ...
    setTimeout(() => {
      setPlan({
        title: "AI-Constructed Push/Pull/Legs – 5 Day Microcycle",
        blocks: [
          {
            label: "Push – Neural priming",
            detail:
              "RPE 7–8 • Incline DB press, machine press, cable fly; finish with shoulder stability work.",
          },
          {
            label: "Pull – Lat bias",
            detail:
              "Tempo rows, neutral-grip pulldowns, rear delt clusters, grip endurance finisher.",
          },
          {
            label: "Legs – Strength & power",
            detail:
              "Back squats, hack squats, RDLs; finish with sleds and anti-rotation core.",
          },
          {
            label: "Conditioning",
            detail:
              "2x per week zone 2 blocks + 1 HIIT ladder aligned with recovery status.",
          },
        ],
      });
      setLoading(false);
        ...
        <form onSubmit={handleGenerate} className="space-y-3 text-xs">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-white/70">Primary goal</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-white/70">Days per week</label>
              <input
                type="number"
                min={1}
                max={7}
            ...
            <div className="space-y-1">
              <label className="text-white/70">Session length (min)</label>
              <input
                type="number"
                min={20}
                max={120}
          ...
          <div className="space-y-1">
            <label className="text-white/70">Available equipment</label>
            <textarea
              rows={3}
              placeholder="Racks, cables, dumbbells to 40kg, sled, rower..."
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? "Building cycle..." : "Generate plan"}
        ...
        <div className="relative flex-1 rounded-3xl border border-white/20 bg-gradient-to-br from-white/10 via-white/5 to-transparent px-5 py-4 text-xs overflow-hidden">
          ...
          {plan ? (
            ...
            <p className="text-[11px] text-white/50">
              Fill out the form on the left and the AI engine will construct a
              structured microcycle here, ready to print or send to members.
            </p>
          )}
        </div>
      </section>
